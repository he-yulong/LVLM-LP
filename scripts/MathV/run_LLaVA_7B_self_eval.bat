@echo off
REM Run LLaVA-7B self-evaluation on MathVista (Windows version, single GPU)

REM Set GPU to use (0 = first GPU)
set CUDA_VISIBLE_DEVICES=0

REM Define temporary output folder
if not exist output\tmp (
    mkdir output\tmp
)

REM Define final output file
set OUTPUT_FILE=output\LLaVA-7B\MathV_self_eval.jsonl
